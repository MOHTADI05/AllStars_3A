...
import pi.dao.classes.ProduitDAO;
...
import pi.utils.BadWordsFilter;
import pi.utils.MailTools;
...
public class ProdBackController implements Initializable {
    ...
    private void AddProduit(ActionEvent event) {
        ...
        if (BadWordsFilter.containsBadWords(userInput)) {
            ...
            Utilisateur utilisateur = Session.getInstance().getLoggedUser();
            try {
                MailTools.sendMail(utilisateur, "produit a ete ajouter avec sucess de compte", "");
            } catch (Exception ex) {
